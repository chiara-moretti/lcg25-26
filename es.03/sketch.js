...
// variabili per l'animazione delle braccia
let armRotation = 0;
...
function draw() {
  ...
  // aggiorna la rotazione delle braccia
  armRotation += rotationSpeed;

  //braccia animate con translate, rotate e scale - dietro al corpo
  fill("#f5e35f");
  
  // braccio sinistro
  push();
  translate(170, 350); // punto di ancoraggio traslato verso il centro
  rotate(sin(armRotation) * 0.2); // rotazione su e giù
  // disegna il braccio sinistro
  quad(0, 0, -2, -25, -75, -110, -75, -90);
  // mano sinistra
  push();
  translate(-75, -100); // posizione della mano
  rotate(PI / -6 + sin(armRotation) * 0.2); // rotazione della mano
  ellipse(0, 0, 25, 40);
  pop();
  pop();
  ...
  // braccio destro
  push();
  translate(330, 350); // punto di ancoraggio traslato verso il centro
  rotate(-sin(armRotation) * 0.2); // rotazione opposta al braccio sinistro
  // disegna il braccio destro
  quad(0, 0, 2, -25, 75, -110, 75, -90);
  // mano destra
  push();
  translate(75, -100); // posizione della mano
  scale(-1, 1); // specchia la mano
  rotate(PI / -6 + sin(armRotation) * 0.3); // stessa rotazione della mano sinistra ma specchiata
  ellipse(0, 0, 25, 40);
  pop();
  pop();
